...
class TestHideSuccess:
    '''Collects test cases for the hide command in which the target post is successfully hidden.
    
    There are three ways this can happen; in the standard, 'as expected' manner, where there is 
    one entry for the targeted post in Post and one entry for the targeted post's author in Account;
    and with separate behaviors if there are multiple entries corresponding to the post or the author.

    These cases should all result in the modification of at least one Post entry (to hide the targeted 
    post) and one Account entry (to increment the author's hidden_count by one); the creation of 
    both a BotAction and ModAction entry; and a Bluesky API call to send a post to the instigating 
    moderator indicating action success, and giving some relevant information.
    '''
    def test_success(self, test_db_conn, mock_client):
    ...
    def test_success_multiple_author_entries(self, test_db_conn, mock_client):
        '''Tests success case in which target post's author has multiple entries in Account table
        
        In this case, only the first account entry (as determined by order in the Account table)
        should have its hidden_count incremented.
        '''
    ...
    def test_success_multiple_post_entries(self, test_db_conn, mock_client):
        '''Tests success case in which target post has multiple entries in Post table
        
        In this case, only the first post entry (as determined by order in the Post table)
        should be hidden.
        '''
...
class TestHideFailure:
    '''Collects test cases for the hide command in which the target post is not successfully hidden.
    
    There are four ways this can happen. In order of which will cause the failure first: the instigating 
    account does not have sufficient moderator level to access the hide command; the targeted post's 
    author is not registered to post in the feeds (has no entry in Account); the targeted post is not 
    in the feeds (has no entry in Post); or, finally, the post is already hidden.

    These cases should all result in no modifications to any Post or Account entry, the creation of a 
    BotAction entry but not a ModAction entry, and a Bluesky API call to send a post to the instigating 
    account indicating action failure, and the reason for the failure.
    '''
    def test_failure_insufficient_mod_level(self, test_db_conn, mock_client):
        '''Tests insufficient mod access failure case.
        
        For this case, the BotAction entry that is created should have authorized=False.
        '''
        # connect & collect
        with DBConnection():
            target_post_before = Post.select().where(Post.hidden)[-1]
    ...
    def test_failure_author_not_signed_up(self, test_db_conn, mock_client):
    ...
    def test_failure_post_not_in_feeds(self, test_db_conn, mock_client):
    ...
    def test_failure_post_already_hidden(self, test_db_conn, mock_client):
        '''Tests failure case in which target post is already hidden.
        
        For this case, we check whether the post is hidden after execution, rather 
        than simply that it's hidden state is the same as before execution as in 
        the other failure cases; because we specifically want this to trigger only 
        on a hidden post and not to unhide it.
        '''
        # connect & collect
        with DBConnection():
            target_post_before = Post.select().where(Post.hidden)[-1]
