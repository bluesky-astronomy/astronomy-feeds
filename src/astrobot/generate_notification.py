...
def construct_facet_mention(
        did : str = "mention author did",
        py_type : str = "app.bsky.richtext.facet#mention"
...
def construct_facet_byteslice(
    ...
    return models.app.bsky.richtext.facet.ByteSlice(byte_end=byte_end, byte_start=byte_start, py_type=py_type)
...
def construct_facet_main(
    ...
    return models.app.bsky.richtext.facet.Main(features=features, index=index, py_type=py_type)
...
def construct_post_record(
    ...
    return models.app.bsky.feed.post.Record(created_at=created_at, text=text, embed=embed, entities=entities, facets=facets, labels=labels, langs=langs, reply=reply, tags=tags, py_type=py_type)
...
def build_reply_ref(
    ...
    reply_ref = models.app.bsky.feed.post.ReplyRef(parent=parent_ref, root=root_ref, py_type=py_type)
...
def build_profileview(
    ...
    if viewer is None: viewer = models.app.bsky.actor.defs.ViewerState(blocked_by=False, blocking=None, blocking_by_list=None, followed_by=None, following=None, known_followers=None, muted=False, muted_by_list=None, py_type='app.bsky.actor.defs#viewerState')

    author = models.app.bsky.actor.defs.ProfileView(did=did, handle=handle, associated=associated, avatar=avatar, created_at=created_at, description=description, display_name=display_name, indexed_at=indexed_at, labels=labels, viewer=viewer, py_type=py_type)
...
def build_notification(
    ...
    if not notification_type in supported_notification_types:
        raise ValueError(f"build_notification: passed unsupported type={notification_type}; can only generate notifications of types in {supported_notification_types}")
    ...
    match notification_type:
        ...
        case "like":
            ...
            if record is None:
                subject = construct_strong_ref_main(cid="liked post cid", uri="liked post uri")
        ...
        case _:
            ...
            raise ValueError(f"build_notification: passed type={notification_type}, cannot handle.")
